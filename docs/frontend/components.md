...
- `<a class="js-phone" href="tel:…">…</a>` / `<a class="js-email" href="mailto:…">…</a>` —
  `ui.js initPhones()` подставляет телефон/email из `config.js` (href всегда; видимый
  текст — только если это сам номер/адрес, поэтому кнопки «Позвонить» и `<span>` с часами
  не затрагиваются). Хардкод в разметке — fallback для no-JS/SEO. Местный номер офиса
  (+7 812…) класс НЕ носит — это отдельный номер.
