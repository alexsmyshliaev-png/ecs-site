...
## Статус на 2026-07-08 (обновлено)

- **Задача A (calc.js)** — ✅ сделано (коммит «Этап 3: calc.js…»).
- **Задача B (соц-иконки)** — ✅ ничего не требовалось: иконки квадратные, `width/height` уже стоят.
- **Задача C (Этап 4, код)** — ✅ сделано: utm.js (`propagate`+yclid/gclid), GTM-загрузчик, `form_start`, контакты из config (`initPhones`/`js-phone`/`js-email`).
- **Задача D1–D3** — ✅ сделано: `CLAUDE.md`, `docs/ENVIRONMENT.md`, `docs/UI_GUIDELINES.md`, `.claude/` в git.
- **D4 (Prettier/CI)** — отложено до Eleventy (согласовано с владельцем).
