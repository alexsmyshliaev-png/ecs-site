...
## [2026-07-08] — Этап 5: поддерживаемость (CLAUDE.md, docs, .claude в git)

### Added
- `CLAUDE.md` (корень) — контекст проекта под новую архитектуру (без FastAPI/pytest): что и где устроено (b24-формы, calc.js, аналитика, контакты), критические инварианты (build_pages, id только в config, ПДн не в localStorage, гейт согласия, порядок скриптов), правила бренда, особенности окружения (нет node, кириллический путь, JXA-проверка синтаксиса), правила коммитов. 62 строки.
- `docs/ENVIRONMENT.md` — карта внешних систем и точек конфигурации: домен/хостинг, Битрикс24 (CRM-формы, виджет), Метрика/GTM/GA4, калькуляторы, контакты/Карты, Вебмастер/Search Console.
- `docs/UI_GUIDELINES.md` — правила бренда текстом: красный `#e60023` (одна главная кнопка на экран), логотип (не менять; партнёрские — grayscale ~0.78), шрифт Inter (self-hosted), токены, доступность.
- `.claude/launch.json` — конфигурация запуска `ecs-site` теперь версионируется.

### Changed
- `.gitignore` — вместо игнорирования всего `.claude/` игнорируется только `.claude/settings.local.json` (конфигурация проекта версионируется).
- `docs/frontend/components.md` — добавлено соглашение `.js-phone`/`.js-email` (`ui.js initPhones()`).
...
### Changed (контакты из config — задел под коллтрекинг)
- `frontend/assets/js/ui.js` — `initPhones()`: телефон/email проставляются из `config.js` в ссылки `.js-phone`/`.js-email` (href всегда; видимый текст — только если это сам номер/адрес, поэтому кнопки «Позвонить» и `<span>` с часами в шапке не затрагиваются). Захардкоженные значения в разметке остаются fallback'ом для no-JS и SEO. Позволяет менять номер в одном месте (условие для подмены номера коллтрекингом).
- `index.html` (+ все страницы через `build_pages.py`), `404.html`, `kasko.html`, `property.html`, `business.html` — основным телефонам (шапка, мобменю, футер, CTA «Позвонить») проставлен класс `js-phone`, email в футере — `js-email`. **Местный номер офиса** `+7 812 240-90-80` (office.html) намеренно оставлен без класса — это отдельный номер.
